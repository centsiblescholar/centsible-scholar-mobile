...
5. **Web subscriber guard:** At the top of the screen content (before plan cards), add a check:
   - ONLY show guard when `isActive === true && platform === 'stripe'` (active web subscription)
   - Guard displays:
     ...
     - Do NOT show plan cards or purchase buttons
   - Return early (don't render plan cards)
   - For all other cases (isActive !== true, platform !== 'stripe', isActive === null, platform === null), show plan cards normally
