...
<objective>
Create the design system foundation: ThemeContext for dark mode support, reusable UI state components (skeleton, empty, error, loading overlay), and ESLint enforcement to prevent future hardcoded colors.

Purpose: All subsequent UI work (Plans 02-04) builds on these primitives. ThemeContext enables dark mode and provides the `useTheme()` hook that replaces static `colors` imports. The UI state components provide consistent loading/error/empty patterns across all screens.

Output: ThemeContext provider, useTheme hook, createCommonStyles factory, 4 reusable UI components, ESLint config with no-color-literals rule, all required npm packages installed.
</objective>
...
<task type="auto">
  <name>Task 2: Create reusable UI state components + ESLint config</name>
  <files>
    src/components/ui/SkeletonCard.tsx
    src/components/ui/EmptyState.tsx
    src/components/ui/ErrorState.tsx
  ...
  <done>
    SkeletonCard, SkeletonList, DashboardSkeleton, EmptyState, ErrorState, and LoadingOverlay components exist with theme-aware coloring. ESLint config warns on hardcoded colors.
  </done>
</task>
