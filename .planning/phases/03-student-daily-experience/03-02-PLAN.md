...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/components/daily/BehaviorStep.tsx"
      provides: "10-category behavior self-assessment with 1-5 button grid (not sliders)"
      min_lines: 40
...
<task type="auto">
  ...
  <action>
    ...
    **2. `src/components/daily/BehaviorStep.tsx`:**
    ...
    - Each category uses the same `ScoreRow` pattern from `behavior.tsx`: label + description + 5 tap-to-select buttons (1-5) with color coding (1=red, 2=orange, 3=yellow, 4=blue, 5=green)
    - BUT: use a more compact layout than the full behavior screen since this is a wizard step. Reduce padding, use smaller text, and make the score buttons slightly smaller (but still 44pt min touch target)
    ...
    - Wrap in ScrollView since 10 categories will scroll
  ...
  <verify>
    ...
    - `src/components/daily/BehaviorStep.tsx` exists with all 10 categories (diet, exercise, work, hygiene, respect, responsibilities, attitude, cooperation, courtesy, service)
    - `src/components/daily/CompletionCelebration.tsx` exists and imports `ConfettiCannon`
  </verify>
  <done>Three wizard step components created: QODStep with question/answer/feedback UI, BehaviorStep with all 10 categories in compact layout, CompletionCelebration with confetti and encouraging message.</done>
</task>
...
- All 10 behavior categories present in BehaviorStep
...
- All 10 behavior categories included with 1-5 button scoring
</success_criteria>
